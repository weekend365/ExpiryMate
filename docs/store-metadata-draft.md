...
보관합니다. 2026-08-29에 노출된 기존 비밀번호 교체와 Connect 로그인 정보 갱신을
완료했습니다.

2026-08-29 사용자 확인으로 심사 계정 로그인, 홈 정상 표시, 설정의
`함께 쓰는 냉장고` 진입점 노출과 `Apple 심사 냉장고` 생성까지 완료했습니다. 심사
계정이 소유자이며 `1명이 함께 써요 · 지금 보는 중` 상태입니다. 앱 재실행 후 세션 유지,
두 번째 구성원의 1회용 코드 수락도 완료해 소유자 화면에서 `2명이 함께 써요`를
확인했습니다. 앱 재실행 후 세션 유지, 반대 방향 재고 수정 동기화, 처분 데이터 준비는
남아 있습니다.
...
준비 조건을 충족했습니다. 앱 강제 종료·재실행 후 상태 유지 확인만 남아 있습니다.

빌드 전에는 현재 운영 앱으로 다음 데이터만 준비합니다.
...
- [x] 심사 전용 데모 이메일 계정 준비와 노출된 비밀번호 교체
...
빌드 일정: **2026-09-01**. 저작권 `2026 devnamu` 수정과 노출된 기존 심사 계정
비밀번호·App Store Connect 로그인 정보 갱신은 2026-08-29 완료했습니다.
