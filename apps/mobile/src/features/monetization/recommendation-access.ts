...
export function recommendationCtaQuotaLabel(access: RecommendationAccess) {
  if (access.tier !== "free") {
    const quota = access.subscriptionQuota;
    return quota
      ? `오늘 ${quota.daily.remaining}회`
      : `오늘 ${access.remaining}회`;
  }
  ...
  if (freeRemaining > 0 && bonus > 0) {
    return `무료 ${freeRemaining}회 · 추천권 ${bonus}회`;
  }
