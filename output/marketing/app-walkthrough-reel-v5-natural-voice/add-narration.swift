...
let videoURL = root.deletingLastPathComponent().appendingPathComponent("app-walkthrough-reel-v3/jango-walkthrough-reel.mp4")
let speechURL = root.appendingPathComponent("jango-narration.wav")
let encodedAudioURL = root.appendingPathComponent("jango-narration.m4a")
let outputURL = root.appendingPathComponent("jango-walkthrough-delicate.mp4")
...
let report: [String: Any] = ["passed": passed, "video_samples_unchanged": before == after, "video_sample_count": after.1, "video_digest": after.0, "duration_seconds": finalAsset.duration.seconds, "width": originalVideo.naturalSize.width, "height": originalVideo.naturalSize.height, "audio_decoded_samples": decodedSamples, "audio_peak": peak, "audio_peak_dbfs": 20 * log10(peak), "audio_decode_completed": audioReader.status == .completed, "voice": "AI Voice Generator delicate (user-selected)", "output": outputURL.path]
