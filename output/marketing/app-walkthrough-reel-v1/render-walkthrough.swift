...
let segments:[Segment]=[
 .init(id:"hook",sourceStart:80.92,sourceEnd:81.65,frames:60,chapter:"재료 등록부터 요리 추천까지",caption:"남은 두부로\n오늘 뭐 먹지?",crop:CGRect(x:34,y:400,width:1138,height:1760),note:"추천 결과 미리보기"),
 .init(id:"open-entry",sourceStart:13.0,sourceEnd:15.1,frames:42,chapter:"01  재료 등록",caption:"냉장고 재료부터 넣어요",crop:CGRect(x:22,y:690,width:1162,height:1880),note:"입력 방식 선택"),
 .init(id:"name",sourceStart:15.1,sourceEnd:19.85,frames:90,chapter:"01  재료 등록",caption:"재료 이름을 넣고",crop:CGRect(x:30,y:510,width:1146,height:2060),note:"입력 구간 1.58배속"),
 .init(id:"date",sourceStart:19.85,sourceEnd:22.75,frames:72,chapter:"01  재료 등록",caption:"유통기한을 골라주세요",crop:CGRect(x:30,y:510,width:1146,height:2060),note:"날짜 선택 1.21배속"),
 .init(id:"saved",sourceStart:22.75,sourceEnd:24.7,frames:54,chapter:"01  재료 등록",caption:"두부 등록, 끝!",crop:CGRect(x:45,y:400,width:1116,height:1500),note:"등록 결과 확인"),
 .init(id:"settings",sourceStart:40.8,sourceEnd:44.65,frames:60,chapter:"02  요리 추천",caption:"어떤 재료로 요리할까요?",crop:CGRect(x:24,y:420,width:1158,height:2150),note:"추천 설정 진입"),
 .init(id:"ingredients",sourceStart:44.65,sourceEnd:55.75,frames:90,chapter:"02  요리 추천",caption:"추천에 쓸 재료를 골라요",crop:CGRect(x:24,y:440,width:1158,height:2130),note:"재료 선택 구간 3.7배속"),
 .init(id:"apply",sourceStart:55.75,sourceEnd:60.9,frames:54,chapter:"02  요리 추천",caption:"조건을 정하고 추천받기",crop:CGRect(x:24,y:420,width:1158,height:2150),note:"조건 적용 2.86배속"),
 .init(id:"wait-start",sourceStart:60.9,sourceEnd:62.1,frames:20,chapter:"02  요리 추천",caption:"장고가 요리를 고르는 중",crop:CGRect(x:40,y:510,width:1126,height:1620),note:"대기 구간 편집"),
 .init(id:"wait-end",sourceStart:75.9,sourceEnd:78.8,frames:42,chapter:"02  요리 추천",caption:"장고가 요리를 고르는 중",crop:CGRect(x:24,y:470,width:1158,height:2100),note:"중간 대기 13.8초 생략"),
 .init(id:"results",sourceStart:78.8,sourceEnd:81.4,frames:75,chapter:"02  요리 추천",caption:"두부 표고 김치전, 어때요?",crop:CGRect(x:24,y:600,width:1158,height:1710),note:"실제 추천 결과"),
 .init(id:"recipe",sourceStart:81.4,sourceEnd:86.5,frames:90,chapter:"03  레시피 확인",caption:"필요한 재료와 조리법까지",crop:CGRect(x:28,y:400,width:1150,height:2170),note:"추천 요리 상세 1.7배속"),
 .init(id:"cooking-step",sourceStart:93.5,sourceEnd:96.0,frames:66,chapter:"03  레시피 확인",caption:"한 단계씩 따라 해보세요",crop:CGRect(x:30,y:405,width:1146,height:2140),note:"실제 조리 안내 화면"),
 .init(id:"cooking-timer",sourceStart:96.0,sourceEnd:98.25,frames:45,chapter:"03  레시피 확인",caption:"순서와 타이머도 함께",crop:CGRect(x:30,y:405,width:1146,height:2140),note:"조리 단계 시연, 실제 조리 완료 아님")
]
...
func makeFrame(segment:Segment?,time:Double,elapsed:Double,context:CGContext)->Double? {
 ...
 do {
   ...
   guard let crop=cg.cropping(to:s.crop) else {return nil}
   // PQ screen recordings use an HDR transfer curve. Convert to SDR and lift
   // exposure consistently, preserving all original UI pixels and text.
   let ci=CIImage(cgImage:crop,options:[.toneMapHDRtoSDR:false,.contentHeadroom:1.0])
   ...
   let scale=min(936/CGFloat(converted.width),1460/CGFloat(converted.height))
   let w=CGFloat(converted.width)*scale,h=CGFloat(converted.height)*scale
   let x:CGFloat=72+(936-w)/2, y:CGFloat=300+(1460-h)/2
   let frame=rectTop(x,y,w,h)
   context.saveGState()
   context.addPath(CGPath(roundedRect:frame,cornerWidth:28,cornerHeight:28,transform:nil));context.clip()
   context.draw(converted,in:frame)
   context.restoreGState()
   context.setStrokeColor(CGColor(red:0.83,green:0.86,blue:0.82,alpha:1));context.setLineWidth(2)
   context.addPath(CGPath(roundedRect:frame,cornerWidth:28,cornerHeight:28,transform:nil));context.strokePath()
 } catch {print("DECODE_ERROR",time,error);return nil}
 text("장고야 부탁해",top:48,size:32,color:green,in:context)
 text(s.chapter,top:103,size:28,color:gray,weight:"SemiBold",in:context)
 let lines=s.caption.split(separator:"\n").map(String.init)
 for (i,line) in lines.enumerated(){text(line,top:151+CGFloat(i)*66,size:s.id=="hook" ?  sixtyFour:54,color:i==1 ? green:ink,in:context)}
 context.setFillColor(mint);context.fill(rectTop(72,1810,916,5))
 context.setFillColor(green);context.fill(rectTop(72,1810,916*CGFloat(elapsed/(Double(bodyFrames)/30)),5))
 let footer=s.id.hasPrefix("wait") ? "실제 앱 화면 · 추천 대기 구간 편집" : "실제 앱 화면 · 입력·선택 구간 배속"
 text(footer,top:1841,size:25,color:gray,weight:"Regular",in:context)
...
var cursor=0
for s in segments {
 timeline.append(["id":s.id,"source_start":s.sourceStart,"source_end":s.sourceEnd,"output_start":Double(cursor)/30,"output_end":Double(cursor+s.frames)/30,"speed":(s.sourceEnd-s.sourceStart)/(Double(s.frames)/30),"caption":s.caption,"chapter":s.chapter,"note":s.note,"crop":[s.crop.origin.x,s.crop.origin.y,s.crop.width,s.crop.height]])
 cursor+=s.frames
}
...
for f in 0..<totalFrames {
 try autoreleasepool {
  ...
  if let actual {entry["source_requested"]=sourceTime;entry["source_actual"]=actual}
  audit.append(entry)
