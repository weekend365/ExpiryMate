...
let width=1206,height=2622,fps=30
let endFrames=120
...
func makeFrame(segment:Segment?,time:Double,elapsed:Double,context:CGContext)->Double? {
 ...
 guard let s=segment else {
   // Fit the existing endcard to the full native-width output.
   let ratio=CGFloat(endcard.height)/CGFloat(endcard.width)
   let cardHeight=CGFloat(width)*ratio
   context.draw(endcard,in:rectTop(0,(CGFloat(height)-cardHeight)/2,CGFloat(width),cardHeight))
   return nil
 }
 ...
 do {
   ...
   context.draw(converted,in:rectTop(0,0,1206,2622))
 } catch {print("DECODE_ERROR",time,error);return nil}
 // Fixed lower subtitle panel, above the bottom action buttons.
 // Its geometry is constant, including the opening two-line hook.
 let panel=rectTop(78,2140,1050,144)
 ...
 let top:CGFloat=lines.count == 2 ? 2154 : 2182
 for (i,line) in lines.enumerated(){
   text(line,top:top+CGFloat(i)*60,size:54,color:CGColor(gray:1,alpha:1),center:true,in:context)
 }
...
for f in 0..<totalFrames {
 try autoreleasepool {
  ...
  if let actual {entry["source_requested"]=sourceTime;entry["source_actual"]=actual;entry["display_frame"]=[0.0,0.0,1206.0,2622.0]}
  audit.append(entry)
