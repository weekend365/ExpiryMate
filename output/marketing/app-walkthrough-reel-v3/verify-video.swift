...
while let sample = output.copyNextSampleBuffer() {
    ...
    if let buffer = CMSampleBufferGetImageBuffer(sample) {
        dimensionsValid = dimensionsValid && CVPixelBufferGetWidth(buffer) == 1206 && CVPixelBufferGetHeight(buffer) == 2622
...
let geometry = map.compactMap { $0["display_frame"] as? [Double] }
let fixedGeometry = geometry.count == expectedFrames - 120 && geometry.allSatisfy { $0 == [0.0, 0.0, 1206.0, 2622.0] }
let size = (try FileManager.default.attributesOfItem(atPath: url.path)[.size] as! NSNumber).intValue
let passed = reader.status == .completed && frames == expectedFrames && dimensionsValid && maximumTimestampError < 0.0001 && abs(asset.duration.seconds - Double(frames) / 30.0) < 0.001 && (sourceErrors.max() ?? 99) < 0.06 && fixedGeometry
let report: [String:Any] = [
    ...
    "original_frame_fills_output_without_margins": fixedGeometry, "body_frames_geometry_checked": geometry.count,
